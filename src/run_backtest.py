...
def _attach_value(rows, odds):
    ...
    for r in rows:
        ...
        for market,key in (("over_25","p_over_25"),("btts_yes","p_btts_yes")):
            odds=odds.get((date,home,away,market))
            sig=evaluate_value(market,float(r[key]),odds)
            outcome=(r["home_goals"]+r["away_goals"]>=3) if market=="over_25" else (r["home_goals"]>=1 and r["away_goals"]>=1)
            enriched.append({"date":date,"home_team":home,"away_team":away,"market":market,"probability":float(r[key]),"fair_odds":sig.fair_odds,"market_odds":odds,"edge":sig.edge,"ev":sig.ev,"status":sig.status,"won":bool(outcome)})
    return enriched
